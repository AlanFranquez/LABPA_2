...
public class Presentacion {
                        ...
                        if (returnValue == JFileChooser.APPROVE_OPTION) {
                            ...
                            if (imagenSeleccionada != null) {
                                ...
                                if (nombreArchivo.endsWith(".jpg") || nombreArchivo.endsWith(".png")) {
                                    ...
                                    imagenLabel.setIcon(imagenSelecc); // Aquí asignamos la imagen al JLabel
                                    imagenLabel.setText(""); // Limpiamos el texto, si era necesario
                                } else {
                ...
                registrarButton.addActionListener(new ActionListener() {
                    ...
                    public void actionPerformed(ActionEvent e1) {
                       ...
                       int dia, mes, anio;
                        ...
                        String path = imagenSeleccionada.getAbsolutePath();
                        if(esProveedor) {
                        	try {
        ...
        mntmListarProductos.addActionListener(new ActionListener() {
            public void actionPerformed(ActionEvent e) {
                ...
                for (DtProducto dt : listaP) {
                    ...
                    productoDT.addMouseListener(new MouseAdapter() {
                        @Override
                        public void mouseClicked(MouseEvent e) {
                            ...
                            detallePanel.add(new JLabel("<html><br />Categorías de los productos: " + dt.getCategorias()));

                            List<String> imagenes = dt.getImagenes(); // Usar List<String>
                            if (imagenes != null && !imagenes.isEmpty()) {
                                JPanel imagePanel = new JPanel();
                                imagePanel.setLayout(new GridLayout(0, 2, 10, 10)); // 2 columnas

                                for (String imagenPath : imagenes) { // Cambiar a String
                                    try {
                                        ImageIcon imageIcon = new ImageIcon(imagenPath); // Cargar imagen desde la ruta
                        ...
                        if (path != null) {
                            ...
                            if (selectedNode.isLeaf()) {
                                ...
                                try {
                                    ...
                                    return; // Manejar errores de conversión
                                }

                                DtProducto dt = s.getDtProducto(numRef);

                                // Crear la ventana de detalles del producto
                                JInternalFrame ventanaDetalleProducto = new JInternalFrame("Detalle de Producto", true, true, true, true);
                                ventanaDetalleProducto.setSize(600, 400);
                                ventanaDetalleProducto.setLayout(new BorderLayout());
                                ...
                                List<String> imagenes = dt.getImagenes(); // Obtener lista de rutas de imágenes
                                if (imagenes != null && !imagenes.isEmpty()) {
                                    JPanel imagePanel = new JPanel();
                                    imagePanel.setLayout(new GridLayout(0, 2, 10, 10)); // 2 columnas, espaciado de 10px
                ...
                tree.addMouseListener(new MouseAdapter() {
                    ...
                    public void mouseClicked(MouseEvent e) {
                        ...
                        if (path != null) {
                            ...
                            if (selectedNode.isLeaf()) {
                                String selection = (String) selectedNode.getUserObject();
                                if (selection.equals("Sin Elementos")) { // Cambiar a equals para comparar strings
                                    return;
                                }
                                ...
                                try {
                                    ...
                                    return; // Manejar error de conversión
                                }
                                ...
                                if (imagenes != null && !imagenes.isEmpty()) {
                                    ...
                                    for (String imagenPath : imagenes) {
                                        try {
                                            ImageIcon imageIcon = new ImageIcon(imagenPath); // Usar la ruta directamente
                                            Image imagenAjuste = imageIcon.getImage();
                                            Image reajuste = imagenAjuste.getScaledInstance(200, 100, Image.SCALE_SMOOTH);
                                            ImageIcon imagenIconAjustada = new ImageIcon(reajuste);
                                            JLabel imageLabel = new JLabel(imagenIconAjustada);
    ...
    private void mostrarDetallesCliente(DTCliente cliente) {
        ...
        String rutaImagen = cliente.getImagenes();
        if (rutaImagen != null && !rutaImagen.isEmpty()) {
            ...
            if (imagenFile.exists()) {
                ...
                panel.add(new JLabel("La imagen no existe en la ruta especificada."));
            }
        } else {
        ...
        panel.add(createLabelValuePair("Tipo de Usuario:", cliente.getTipo()));
        panel.add(createLabelValuePair("Mail:", cliente.getCorreo()));
        panel.add(createLabelValuePair("Nick:", cliente.getNick()));
        panel.add(createLabelValuePair("Nombre Completo:", cliente.getNombre() + " " + cliente.getApellido()));
        panel.add(createLabelValuePair("Fecha de Nacimiento:", 
            cliente.getNacimiento().getDia() + " - " + cliente.getNacimiento().getMes() + " - " + cliente.getNacimiento().getAnio()));

        panel.add(Box.createVerticalStrut(5));
        panel.add(new JLabel("Órdenes: "));
        
        if (cliente.getOrdenes().isEmpty()) {
            panel.add(new JLabel("   Todavía no existen órdenes"));
        } else {
    ...
    private void mostrarDetallesProveedor(DTProveedor proveedor) {
        ...
        String rutaImagen = proveedor.getImagen();
        if (rutaImagen != null && !rutaImagen.isEmpty()) {
            File imagenFile = new File(rutaImagen);
            if (imagenFile.exists()) {
                ImageIcon imagenIcon = new ImageIcon(rutaImagen);
        ...
        // Usar createLabelValuePair para agregar detalles
        panel.add(createLabelValuePair("Tipo de Usuario:", "Proveedor"));
        panel.add(createLabelValuePair("Mail:", proveedor.getCorreo()));
        panel.add(createLabelValuePair("Nick:", proveedor.getNick()));
        panel.add(createLabelValuePair("Nombre Completo:", proveedor.getNombre() + " " + proveedor.getApellido()));
        panel.add(createLabelValuePair("Fecha de Nacimiento:", 
            proveedor.getNacimiento().getDia() + " - " + proveedor.getNacimiento().getMes() + " - " + proveedor.getNacimiento().getAnio()));
        panel.add(createLabelValuePair("Compañía:", proveedor.getCompania()));
        panel.add(createLabelValuePair("Link:", proveedor.getLink()));
        panel.add(Box.createVerticalStrut(5));
